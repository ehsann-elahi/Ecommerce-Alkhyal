$(document).ready(function() {
	$(".mahardhi-search input[name='search']").autocomplete({
		source: function(request, response) {
			...
			const url = "index.php?route=extension/mahardhi/module/mtsearch.filter&filter_name=" + request.term + "&filter_category=" + categoryId;
			$.ajax({
				url,
				data: {
		...
		appendTo: '#sResultContainer'
	});

	$.ui.autocomplete.prototype._renderItem = function(ul, item) {
		var elements = [];
		var field_html1 = '<div class="images"><img class="product-image img-fluid" src="' + item.image + '" /></div>';
